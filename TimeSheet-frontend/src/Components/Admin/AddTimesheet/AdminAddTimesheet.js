...
useEffect(() => {
  const loadSavedData = () => {
    const savedData = JSON.parse(localStorage.getItem(adminId)) || [];
...
const handleSaveTimesheetData = () => {
  ...
  disabled={
    ...
    entry.date > new Date(new Date().setHours(23, 59, 59, 999)) // Disable only future dates
  }
  onChange={(e) =>
